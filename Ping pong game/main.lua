...
function love.keypressed(key)
    ...
    elseif gameState == 'serve' then
        ...
        ball.dx = 100 + math.random(30)
        ball.dy = 40 + math.random(80)
...
function love.draw()
    push:start()
    love.graphics.clear(40/255, 85/255, 52/255 , 255/255)
...
function updatePlayers(dt)
    ...
    elseif love.keyboard.isDown('s') and player1.y < 184 then 
        player1.y = player1.y + PADDLE_SPEED * dt
    end

    if love.keyboard.isDown('up') and player2.y > 0 then 
        player2.y = player2.y - PADDLE_SPEED * dt
    elseif love.keyboard.isDown('down') and player2.y < 184 then 
        player2.y = player2.y + PADDLE_SPEED * dt
    end
end
...
function drawTitle()
    love.graphics.setFont(LARGE_FONT)
    love.graphics.printf('Pong Game ', 0, 10, VIRTUAL_WIDTH, 'center')
    love.graphics.setFont(SMALL_FONT)
    love.graphics.printf('Press Enter for ', 0, VIRTUAL_HEIGHT - 32, VIRTUAL_WIDTH, 'center')
    love.graphics.printf('For fun', 0, VIRTUAL_HEIGHT - 18, VIRTUAL_WIDTH, 'center')
end
...
function drawBall()
    love.graphics.setColor(150,0,125)
